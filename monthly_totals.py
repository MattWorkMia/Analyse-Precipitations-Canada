...
def pivot_months_pandas(data: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    sdata = data.drop(['station','latitude','longitude','elevation'], axis=1)
    sdata = sdata.rename(columns={'date' : 'month'})
    sdata['month'] = sdata['month'].map(lambda d: date_to_month(d))
    monthly = sdata.groupby(['name','month']).sum().reset_index()
    counts = sdata.groupby(['name','month']).count().reset_index()
...
def compute_pairwise(df: pd.DataFrame, func: callable) -> pd.DataFrame:
    ...
    **chaque paire de lignes du dataframe**.
    ...
    ```
    """
    return pd.DataFrame(squareform(pdist(df, func)),index = df.index, columns= df.index)
...
def compute_pairwise_distances(df: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    new_df = df.pivot_table(index='name',values=['latitude','longitude'])
...
def correlation(u, v) -> float:
    ...
    f_u = u[idx]
    f_v = v[idx]
    corr = ((f_u - f_u.mean()) * (f_v - f_v.mean())).mean() / (f_u.std() * f_v.std())
